import { ArrowLeft, Building2, Calendar, CheckCircle2, ChevronRight, Clock, ExternalLink, Globe, Layout, Mail, MoreVertical, Package, Settings, ShieldCheck, User, Users, Search, X, Edit, Trash2, Key, RefreshCw, AlertCircle, Ban, History, Shield, Zap, FileText } from "lucide-react";
...
export default function ClientViewPage() {
        ...
        return (
            <div className="min-h-screen flex items-center justify-center bg-gray-50 p-6">
                <div className="text-center max-w-md bg-white p-8 rounded-3xl shadow-sm border border-gray-100">
                    ...
                    <p className="text-gray-500 mb-6">The organization you are looking for does not exist or has been removed.</p>
                    <button
    ...
    const onboardingSteps = [
        { id: 1, title: "Company Profile", status: "Completed", date: "Jan 10, 2024", icon: <Building2 size={16} /> },
        { id: 2, title: "Admin Configuration", status: "Completed", date: "Jan 11, 2024", icon: <Shield size={16} /> },
        { id: 3, title: "Module Setup", status: client.onboardingStatus === "Completed" ? "Completed" : "In Progress", date: "Jan 12, 2024", icon: <Package size={16} /> },
        { id: 4, title: "Data Import", status: client.onboardingStatus === "Completed" ? "Completed" : "Pending", date: "-", icon: <FileText size={16} /> },
        { id: 5, title: "Final Review", status: client.onboardingStatus === "Completed" ? "Completed" : "Pending", date: "-", icon: <Zap size={16} /> }
    ];
                    ...
                    <div className="flex items-center gap-4 min-w-0">
                        ...
                        <div className="flex flex-col min-w-0">
                            <div className="flex items-center gap-2">
                                <h1 className="text-base md:text-lg font-bold text-gray-900 truncate">{client.organizationName}</h1>
                                <span className={`hidden sm:inline-flex px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${client.adminLoginStatus === 'Active' ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-gray-100 text-gray-600 border border-gray-200'
                                    }`}>
                                    {client.adminLoginStatus}
                                </span>
                            </div>
                            <p className="text-[11px] md:text-xs text-gray-500 truncate">{client.adminEmail}</p>
                        </div>
                    </div>
                ...
                <div className="flex flex-col lg:flex-row gap-6 lg:gap-10">
                    ...
                    <div className="flex-1 space-y-6 lg:space-y-10 min-w-0">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 md:gap-6">
                            {[
                                { label: "Total Users", value: client.stats.totalUsers, icon: <Users className="text-blue-600" />, trend: "+12%" },
                                { label: "Active Projects", value: client.stats.activeProjects, icon: <Layout className="text-purple-600" />, trend: "Stable" },
                                { label: "Storage Used", value: client.stats.storageUsed, icon: <Globe className="text-emerald-600" />, trend: "60%" },
                                { label: "Last Activity", value: client.stats.lastActivity, icon: <Clock className="text-amber-600" />, trend: "Real-time" },
                            ].map((stat, i) => (
                                <div key={i} className="bg-white p-5 md:p-6 rounded-3xl border border-gray-100 shadow-sm hover:shadow-md transition-all group">
                            ...
                            <div className="p-4 md:p-6">
                                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                    <button className="flex items-center gap-4 p-4 rounded-2xl border border-gray-100 hover:border-blue-200 hover:bg-blue-50/30 transition-all text-left group cursor-pointer">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-gray-900">Resend Welcome Email</p>
                                            <p className="text-xs text-gray-500">Send login credentials to admin</p>
                                        </div>
                                    </button>
                                    ...
                                    <button className="flex items-center gap-4 p-4 rounded-2xl border border-gray-100 hover:border-purple-200 hover:bg-purple-50/30 transition-all text-left group cursor-pointer">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-gray-900">Reset Admin Password</p>
                                            <p className="text-xs text-gray-500">Generate a recovery link</p>
                                        </div>
                                    </button>
                                    ...
                                    <button className="flex items-center gap-4 p-4 rounded-2xl border border-gray-100 hover:border-red-200 hover:bg-red-50/30 transition-all text-left group cursor-pointer">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-gray-900">Enable/Disable Organization</p>
                                            <p className="text-xs text-gray-500">Restrict or grant platform access</p>
                                        </div>
                                    </button>
                                    ...
                                    <button className="flex items-center gap-4 p-4 rounded-2xl border border-gray-100 hover:border-amber-200 hover:bg-amber-50/30 transition-all text-left group cursor-pointer">
                                        ...
                                        <div>
                                            <p className="font-bold text-sm text-gray-900">Audit Logs</p>
                        ...
                        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                            <div className="p-4 md:p-6 border-b border-gray-50 flex items-center justify-between">
                                ...
                                <button className="text-sm text-blue-600 font-bold hover:underline cursor-pointer">Manage Modules</button>
                            </div>
                            <div className="p-4 md:p-6">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    {client.enabledModules.map((module) => (
                                        <div key={module} className="flex items-center justify-between p-4 rounded-2xl bg-gray-50 border border-gray-100 group hover:bg-white hover:shadow-md transition-all">
                                            <div className="flex items-center gap-4">
                                                <div className="w-10 h-10 bg-white shadow-sm rounded-xl flex items-center justify-center text-blue-600 font-bold">
                                                    {module.charAt(0)}
                                                </div>
                                                <div>
                                                    <p className="font-bold text-sm text-gray-900">{module} Module</p>
                                                    <p className="text-[11px] text-gray-500 font-medium">Subscription: {client.purchasedPlan}</p>
                                                </div>
                                            </div>
                                            <div className="flex items-center gap-2">
                        ...
                        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden">
                            <div className="p-6">
                                ...
                                <div className="space-y-6">
                                    {[
                                        { label: "Plan Type", value: client.purchasedPlan, icon: <CheckCircle2 size={16} className="text-blue-500" /> },
                                        { label: "Admin Account", value: client.adminEmail, icon: <Mail size={16} className="text-blue-500" /> },
                                        { label: "Joined On", value: formatDate(client.createdDate), icon: <Calendar size={16} className="text-blue-500" /> },
                                    ].map((item, i) => (
                                        <div key={i} className="flex items-start gap-4">
                                            ...
                                            <div className="min-w-0">
                                                ...
                                                <p className="text-sm font-bold text-gray-900 break-all">{item.value}</p>
                                            </div>
                                        </div>
                                    ))}
                                </div>
                        ...
                        <div className="bg-white rounded-3xl border border-gray-100 shadow-sm overflow-hidden flex flex-col">
                            <div className="p-4 md:p-6 border-b border-gray-50 bg-gradient-to-r from-blue-50/50 to-transparent">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${client.onboardingStatus === 'Completed' ? 'bg-green-50 text-green-700' : 'bg-blue-50 text-blue-700'
                                        }`}>
                                ...
                                <div className="mt-4">
                                    <div className="flex justify-between text-[10px] font-bold text-gray-400 uppercase mb-2">
                                        <span>Progress</span>
                                        <span>{client.onboardingStatus === 'Completed' ? '100%' : '60%'}</span>
                                    </div>
                                    <div className="h-1.5 w-full bg-gray-100 rounded-full overflow-hidden">
                                        <div
                                            className="h-full bg-gradient-to-r from-blue-500 to-indigo-600 rounded-full transition-all duration-1000"
                                            style={{ width: client.onboardingStatus === 'Completed' ? '100%' : '60%' }}
                                        ></div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div className="space-y-6 md:space-y-8">
                                    {onboardingSteps.map((step, idx) => (
                                        <div key={step.id} className="relative flex items-center gap-4 group">
                                            {/* Step Indicator */}
                                            <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold shrink-0 transition-all border-2 z-10 ${step.status === 'Completed'
                                            ...
                                            <div className="flex-1 bg-white">
                                                <div className="flex flex-col">
                                                    <h4 className={`text-xs md:text-sm font-bold transition-colors ${step.status === 'Completed' ? 'text-gray-900' :
                                            ...
                                            {/* Status Tag */}
                                            <div className={`hidden sm:block text-[9px] font-bold px-2 py-0.5 rounded-md uppercase tracking-tight ${step.status === 'Completed' ? 'bg-green-50 text-green-600' :
                                                    step.status === 'In Progress' ? 'bg-blue-50 text-blue-600' : 'bg-gray-50 text-gray-400'
                                                }`}>
                                                {step.status}
                                            </div>
                                        </div>
